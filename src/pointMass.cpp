...
void Eom::init( Eigen::Vector3d Vb,
                Eigen::Vector3d euler_angles,
                Eigen::Vector2d coord_angles,
                double h,
                double dt ) {
    this->dt = dt;
...
void Eom::system( const state_type& x, state_type& dxdt, double t ) {
    ...
    this->rad_acc = -Frames::R_fe( Eigen::Vector2d{ mu, l } ) *
                    w_ei.cross( w_ei.cross( r_e ) );
...
void Eom::simulate(
    double V, double theta, double psi, double fin_alt, double dt ) {
    double t0 = 0.0;
    double t = t0;
    this->dt = dt;
