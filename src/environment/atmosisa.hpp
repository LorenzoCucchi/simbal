...
public:
   ...
   * @return double
   */
  double get_speed_of_sound() { return speed_of_sound; }
...
private:
   ...
   * @return double
   */
  double compute_pressure(double start_temperature, double start_gradient,
                          double next_altitude, double start_altitude,
                          double start_pressure);

  /* Boltzmann constant (N*m/K) */
  const double k = 1.380622e-23;

  /* Molecular weights values of air components */
  const double M[10] = {28.0134, 31.9988, 39.948, 44.00995, 20.183,
                        4.0026,  83.80,   131.30, 16.04303, 2.01594};

  /* Avogadro's number */
  const double N_A = 6.022169e23;

  /* Gas constant (N*m/kmol/K) */
  const double R_star = 8.31432e3;

  /* Fractional volume concentrations (dimensionless) */
  const double F[10] = {0.78084,    0.209476,   0.00934,    0.000314,
  ...
  const double H[8] = {0.0, 11.0, 20.0, 32.0, 47.0, 51.0, 71.0, 84.8520};

  /* Gradients of molecular scale temperature w.r.t. the geopotential height
   * (K/km) */
  const double L_Mb[7] = {-6.5, 0.0, 1.0, 2.8, 0.0, -2.8, -2.0};

  /* Sea-level atmospheric pressure (Pa) */
  const double P_0 = 1.013250e5;

  /* Effective earth's radius (Km) */
  const double r_0 = 6.356766e6;
  ...
  const double sigma = 3.65e-10;

  /* Air gas constant */
  const double r = 287.05;

  /* Molecular weight of the atmosphere, from reference table 3. */
  const double M_0 = 28.0134 * 0.78084 + 31.9988 * 0.209476 + 39.948 * 0.00934 +
                     44.00995 * 0.000314 + 20.183 * 0.00001818 +
                     4.0026 * 0.00000524 + 83.80 * 0.00000114 +
                     131.30 * 0.000000087 + 16.04303 * 0.000002 +
                     2.01594 * 0.0000005;
};