...
void Atmosphere::atmos_press_temp() {
  ...
  for (size_t i = 1; i <= 7; i++) {
    ...
    start_pressure =
        compute_pressure(start_temperature, start_gradient, this->H.at(i),
                         start_altitude, start_pressure);
    start_temperature =
        start_temperature + start_gradient * (this->H.at(i) - start_altitude);
    start_gradient = this->L_Mb.at(i);
    start_altitude = this->H.at(i);
  }
};
...
auto Atmosphere::compute_pressure(double start_temperature,
